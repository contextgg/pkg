...
type GCSBucketParams struct {
	...
	// ProjectId specifies the GCS project that the bucket resides in.
	ProjectId string
}
...
// NewGCSServiceWithCreds returns a GCSService object given a GCloud service account file path.
func NewGCSServiceWithCreds(filename string) (*GCSService, error) {
	ctx := context.Background()
...
func (service *GCSService) CreateBucket(ctx context.Context, params GCSBucketParams) error {
	bh := service.Client.Bucket(params.Bucket)
	...
	if _, err := bh.Attrs(ctx); err != nil {
		...
		if err := bh.Create(ctx, params.ProjectId, nil); err != nil {
			return err
		}
	}
